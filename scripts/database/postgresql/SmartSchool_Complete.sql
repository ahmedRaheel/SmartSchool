/*
 SmartSchool - Complete PostgreSQL Database
 Generated from the latest attached SmartSchool project.

 IMPORTANT:
 - IdentityServer / ASP.NET Identity authentication tables are intentionally NOT created here.
 - User/actor identifiers in business tables remain UUID columns without FK constraints to Identity.
 - Each bounded context/module owns its PostgreSQL schema.
 - PostgreSQL is the default database.
 - pgvector is included for local Ollama RAG.
 - PostgreSQL distributed cache is included as HybridCache L2.
 - Hangfire creates/manages its own persistence tables when configured.
 - Run this script against the SmartSchool database in pgAdmin Query Tool.
*/
...
CREATE TABLE saas.school_branding (
    ...
    logo bytea,
    logo_content_type varchar(150),
    logo_file_name varchar(255),
    small_logo bytea,
    small_logo_content_type varchar(150),
    small_logo_file_name varchar(255),
    favicon bytea,
    favicon_content_type varchar(150),
    favicon_file_name varchar(255),
    certificate_logo bytea,
    certificate_logo_content_type varchar(150),
    certificate_logo_file_name varchar(255),
    letterhead bytea,
    letterhead_content_type varchar(150),
    letterhead_file_name varchar(255),
    watermark bytea,
    watermark_content_type varchar(150),
    watermark_file_name varchar(255),
...
CREATE TABLE student.student (
    ...
    gender varchar(30),
    photo bytea,
    photo_content_type varchar(150),
...
CREATE TABLE transport.driver (
    ...
    tenant_id uuid NOT NULL REFERENCES saas.tenant(tenant_id),
    employee_id uuid REFERENCES hr.employee(employee_id),
    driver_number varchar(60) NOT NULL,
    full_name varchar(200) NOT NULL,
    ...
    picture bytea,
    picture_content_type varchar(150),
    ...
    UNIQUE(tenant_id,cnic_number),
    UNIQUE(tenant_id,driving_license_number)
);

CREATE INDEX ix_driver_tenant_status
ON transport.driver(tenant_id,status);
...
CREATE TABLE transport.vehicle_driver_assignment (
    vehicle_driver_assignment_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL REFERENCES saas.tenant(tenant_id),
    vehicle_id uuid NOT NULL REFERENCES transport.vehicle(vehicle_id),
    driver_id uuid NOT NULL REFERENCES transport.driver(driver_id),
    effective_from date NOT NULL,
    effective_to date,
    is_primary boolean NOT NULL DEFAULT true,
...
CREATE INDEX ix_vehicle_driver_assignment_current
ON transport.vehicle_driver_assignment(tenant_id,vehicle_id,effective_to);
...
-- Duplicate ix_prediction_student_type definition removed; the canonical index is defined earlier in this script.
...
CREATE TABLE IF NOT EXISTS document.DocumentType (
	Id uuid PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS document.StudentDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_StudentDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_StudentDocument_Owner_Type
	ON document.StudentDocument(TenantId, StudentId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_StudentDocument_Hash
	ON document.StudentDocument(TenantId, Sha256Hash);

CREATE TABLE IF NOT EXISTS document.ParentDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_ParentDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_ParentDocument_Owner_Type
	ON document.ParentDocument(TenantId, ParentId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_ParentDocument_Hash
	ON document.ParentDocument(TenantId, Sha256Hash);

CREATE TABLE IF NOT EXISTS document.TeacherDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_TeacherDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_TeacherDocument_Owner_Type
	ON document.TeacherDocument(TenantId, TeacherId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_TeacherDocument_Hash
	ON document.TeacherDocument(TenantId, Sha256Hash);

CREATE TABLE IF NOT EXISTS document.EmployeeDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_EmployeeDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_EmployeeDocument_Owner_Type
	ON document.EmployeeDocument(TenantId, EmployeeId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_EmployeeDocument_Hash
	ON document.EmployeeDocument(TenantId, Sha256Hash);

CREATE TABLE IF NOT EXISTS document.CandidateDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_CandidateDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_CandidateDocument_Owner_Type
	ON document.CandidateDocument(TenantId, CandidateId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_CandidateDocument_Hash
	ON document.CandidateDocument(TenantId, Sha256Hash);

CREATE TABLE IF NOT EXISTS document.DriverDocument (
	Id uuid PRIMARY KEY,
	TenantId uuid NOT NULL,
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_DriverDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
CREATE INDEX IF NOT EXISTS IX_DriverDocument_Owner_Type
	ON document.DriverDocument(TenantId, DriverId, DocumentTypeId);
CREATE INDEX IF NOT EXISTS IX_DriverDocument_Hash
	ON document.DriverDocument(TenantId, Sha256Hash);
...
CREATE TABLE IF NOT EXISTS SchoolDocument (
	...
	UpdatedAt timestamptz NULL,
	RowVersion bytea NOT NULL,
	CONSTRAINT FK_SchoolDocument_DocumentType FOREIGN KEY (DocumentTypeId) REFERENCES document.DocumentType(Id),
...
-- =========================================================
CREATE SCHEMA IF NOT EXISTS reference;
SET search_path TO reference, public;
...
-- =========================================================
CREATE TABLE IF NOT EXISTS communication."NotificationTypeLookup" ("Id" uuid PRIMARY KEY,"Code" varchar(100) NOT NULL UNIQUE,"Name" varchar(150) NOT NULL,"DisplayOrder" integer NOT NULL,"IsActive" boolean NOT NULL DEFAULT true);
...
CREATE TABLE IF NOT EXISTS ai_core."RagKnowledgeDocuments" (
    "Id" uuid PRIMARY KEY,
    "TenantId" uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_core."RagKnowledgeChunks" (
    "Id" uuid PRIMARY KEY,
    "TenantId" uuid NOT NULL,
    ...
    CONSTRAINT "FK_RagKnowledgeChunks_Document"
        ...
        REFERENCES ai_core."RagKnowledgeDocuments"("Id")
        ON DELETE CASCADE,
