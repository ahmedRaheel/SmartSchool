...
public static class UpdateLearningResource
{
	...
	internal sealed class UpdateLearningResourceDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateLearningResource
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR learning_resource_id <> @ExcludingId)
					);
					""";
		...
		public async Task<LearningResourceEntity?> GetByIdAsync(
				...
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<LearningResourceEntity>(
					new CommandDefinition(
						sql,
						new
						{
