...
public static class UpdateLesson
{
	...
	internal sealed class UpdateLessonDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateLesson
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR lesson_id <> @ExcludingId)
					);
					""";
	...
	public sealed class Handler(IUpdateLesson dataAccess)
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(LessonEntity), request.Code)));
			}
	...
	private static Response MapResponse(LessonEntity entity)
	{
		return new Response(
			entity.TenantId,
			entity.LessonId,
