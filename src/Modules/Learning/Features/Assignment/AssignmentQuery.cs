...
/// </summary>
public sealed class AssignmentQuery(IDbConnectionFactory connectionFactory) : IAssignmentQuery
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<AssignmentEntity>(
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT EXISTS (
                ...
                WHERE tenant_id = @TenantId
                  AND code = @Code
                  AND (@ExcludingId IS NULL OR academic_assignment_id <> @ExcludingId)
            );
            """;
