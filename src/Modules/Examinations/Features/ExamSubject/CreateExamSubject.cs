...
public static class CreateExamSubject
{
    ...
    public interface ICreateExamSubjectCommand
    {
        Task AddAsync(
    ...
    internal sealed class CreateExamSubjectCommand(IExaminationsDbContext dbContext) : ICreateExamSubjectCommand
    {
    ...
    public sealed class Handler(ICreateExamSubjectCommand command, IBusinessNumberGenerator numberGenerator)
        : IRequestHandler<Request, Result<Response>>
    {
        public async Task<Result<Response>> HandleAsync(
            ...
            CancellationToken cancellationToken)
        {
            var code = await numberGenerator.NextAsync("ExamSubject", "ES", request.TenantId, 3, cancellationToken);

            var entity = ExamSubjectEntity.Create(
                request.TenantId,
                code,
                request.Name);
