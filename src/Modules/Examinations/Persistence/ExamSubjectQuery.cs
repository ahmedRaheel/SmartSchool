...
/// </summary>
public sealed class ExamSubjectQuery(IDbConnectionFactory connectionFactory) : IExamSubjectQuery
{
	...
	public async Task<PagedResult<ExamSubjectEntity>> GetPageAsync(
		Guid tenantId,
		int page,
		int pageSize,
		CancellationToken cancellationToken)
	{
		...
		var items = (await connection.QueryAsync<ExamSubjectEntity>(
			new CommandDefinition(
				pageSql,
				parameters,
				cancellationToken: cancellationToken)).ConfigureAwait(false))
		...
		return new PagedResult<ExamSubjectEntity>(
			items,
			page,
			pageSize,
			totalCount);
		...
		CancellationToken cancellationToken)
	{
		const string sql = """
			SELECT EXISTS (
				...
				WHERE tenant_id = @TenantId
				  AND code = @Code
				  AND (@ExcludingId IS NULL OR exam_subject_id <> @ExcludingId)
			);
			""";
