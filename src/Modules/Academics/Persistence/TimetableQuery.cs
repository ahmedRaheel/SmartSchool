...
/// </summary>
public sealed class TimetableQuery(IDbConnectionFactory connectionFactory) : ITimetableQuery
{
		...
		CancellationToken cancellationToken)
	{
		...
		return await connection.QuerySingleOrDefaultAsync<TimetableEntity>(
		...
		CancellationToken cancellationToken)
	{
		const string sql = """
			SELECT EXISTS (
				...
				WHERE tenant_id = @TenantId
				  AND code = @Code
				  AND (@ExcludingId IS NULL OR timetable_id <> @ExcludingId)
			);
			""";
