...
/// </summary>
public sealed class ProgramQuery(IDbConnectionFactory connectionFactory) : IProgramQuery
{
		...
		CancellationToken cancellationToken)
	{
		...
		return await connection.QuerySingleOrDefaultAsync<ProgramEntity>(
		...
		CancellationToken cancellationToken)
	{
		...
		var items = (await connection.QueryAsync<ProgramEntity>(
			new CommandDefinition(
				pageSql,
				parameters,
				cancellationToken: cancellationToken))).ConfigureAwait(false)
		...
		CancellationToken cancellationToken)
	{
		const string sql = """
			SELECT EXISTS (
				...
				WHERE tenant_id = @TenantId
				  AND code = @Code
				  AND (@ExcludingId IS NULL OR program_id <> @ExcludingId)
			);
			""";
