...
public static class GetSubjectPage
{
    public sealed record Query(
        Guid TenantId,
        int Page = 1,
        int PageSize = 25) : IRequest<Result<PagedResult<SubjectResponse>>>;

    public sealed class Handler(ISubjectQuery entityQuery)
        : IRequestHandler<Query, Result<PagedResult<SubjectResponse>>>
    {
        public async Task<Result<PagedResult<SubjectResponse>>> HandleAsync(
            Query request,
            CancellationToken cancellationToken)
        {
            ...
            var response = new PagedResult<SubjectResponse>(
                page.Items.Select(SubjectResponse.FromEntity).ToArray(),
                page.Page,
                page.PageSize,
                page.TotalCount);
    ...
    public static IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet(
                ...
                async (Guid tenantId, int page, int pageSize, IMediator mediator, CancellationToken cancellationToken) =>
                {
                    ...
                    var result = await mediator.SendAsync<Query, Result<PagedResult<SubjectResponse>>>(
