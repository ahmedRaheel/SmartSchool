...
public static class UpdateCourseOffering
{
	...
	internal sealed class UpdateCourseOfferingDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateCourseOffering
	{
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(CourseOfferingEntity), request.Code)));
			}
