...
public static class GetAcademicYearPage
{
    public sealed record Query(
        Guid TenantId,
        int Page = 1,
        int PageSize = 25) : IRequest<Result<PagedResult<AcademicYearResponse>>>;

    public sealed class Handler(IAcademicYearQuery entityQuery)
        : IRequestHandler<Query, Result<PagedResult<AcademicYearResponse>>>
    {
        public async Task<Result<PagedResult<AcademicYearResponse>>> HandleAsync(
            Query request,
            CancellationToken cancellationToken)
        {
            ...
            var response = new PagedResult<AcademicYearResponse>(
                page.Items.Select(AcademicYearResponse.FromEntity).ToArray(),
                page.Page,
                page.PageSize,
                page.TotalCount);
    ...
    public static IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet(
                ...
                async (Guid tenantId, int page, int pageSize, IMediator mediator, CancellationToken cancellationToken) =>
                {
                    ...
                    var result = await mediator.SendAsync<Query, Result<PagedResult<AcademicYearResponse>>>(
