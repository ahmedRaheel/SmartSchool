...
public static class GetStudentActivityPage
{
    ...
    {
        public async Task<Result<PagedResult<StudentActivity>>> HandleAsync(
            Query query,
            CancellationToken cancellationToken)
        {
            ...
            return Result<PagedResult<StudentActivity>>.Success(result);
        }
    }
    ...
    {
        endpoints.MapGet(
                "/api/activities/student-activity",
                async (
                    Guid tenantId,
                    int page,
                    int pageSize,
