...
public static class UpdateLoan
{
	...
	internal sealed class UpdateLoanDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateLoan
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR book_loan_id <> @ExcludingId)
					);
					""";
		...
		public async Task<LoanEntity?> GetByIdAsync(
				...
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<LoanEntity>(
					new CommandDefinition(
						sql,
						new
						{
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(LoanEntity), request.Code)));
			}
