...
public static class CreateDiscount
{
	...
	internal sealed class CreateDiscountDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : ICreateDiscount
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR discount_id <> @ExcludingId)
					);
					""";
	...
	public sealed class Handler(ICreateDiscount dataAccess)
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(DiscountEntity), request.Code)));
			}
