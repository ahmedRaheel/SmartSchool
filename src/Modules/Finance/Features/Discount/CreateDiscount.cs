...
public static class CreateDiscount
{
    ...
    internal sealed class CreateDiscountCommand(IFinanceDbContext dbContext) : ICreateDiscountCommand
    {
    ...
    public sealed class Handler(ICreateDiscountCommand command, IBusinessNumberGenerator numberGenerator)
        : IRequestHandler<Request, Result<Response>>
    {
        public async Task<Result<Response>> HandleAsync(
            ...
            CancellationToken cancellationToken)
        {
            var code = await numberGenerator.NextAsync("Discount", "DIS", request.TenantId, 3, cancellationToken);

            var entity = DiscountEntity.Create(
                request.TenantId,
                request.BranchId,
                code,
                request.Name);
