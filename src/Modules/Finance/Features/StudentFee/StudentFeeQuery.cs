...
/// </summary>
public sealed class StudentFeeQuery(IDbConnectionFactory connectionFactory) : IStudentFeeQuery
{
    public async Task<StudentFeeEntity?> GetByIdAsync(
        Guid tenantId,
        Guid id,
        CancellationToken cancellationToken)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<StudentFeeEntity>(
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT EXISTS (
                ...
                WHERE tenant_id = @TenantId
                  AND code = @Code
                  AND (@ExcludingId IS NULL OR student_fee_id <> @ExcludingId)
            );
            """;
