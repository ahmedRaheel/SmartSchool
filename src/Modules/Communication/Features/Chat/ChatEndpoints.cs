...
public static class ChatEndpoints
{
    ...
    private static async Task<IResult> ConversationsAsync(ITenantScope scope,ICommunicationDbContext db,CancellationToken ct)
    {
        var userId=scope.UserId; var tenant=scope.TenantId;
        var participantIds=db.ChatParticipants.AsNoTracking().Where(x=>x.UserId==userId && x.IsActive && (!tenant.HasValue || x.TenantId==tenant.Value)).Select(x=>x.ConversationId);
        var rows=await db.ChatConversations.AsNoTracking().Where(x=>participantIds.Contains(x.ChatConversationId) && x.IsActive)
    ...
    private static async Task<IResult> MessagesAsync(Guid conversationId,ITenantScope scope,ICommunicationDbContext db,CancellationToken ct)
    {
        var conversation=await AuthorizedConversation(conversationId,scope,db,ct); if(conversation is null)return Results.Forbid();
        var rows=await db.ChatMessages.AsNoTracking().Where(x=>x.ConversationId==conversationId && x.TenantId==conversation.TenantId && x.IsActive && !x.IsDeleted)
    ...
    private static async Task<IResult> CreateAsync(CreateConversationRequest request,ITenantScope scope,ICommunicationDbContext db,CancellationToken ct)
    {
        ...
        await db.ChatConversations.AddAsync(entity,ct);
        ...
        foreach(var p in participants) await db.ChatParticipants.AddAsync(ChatParticipantEntity.Create(tenant.Value,entity.ChatConversationId, p.UserId,p.Role),ct);
    ...
    private static async Task<IResult> SendAsync(Guid conversationId,SendRequest request,ITenantScope scope,ICommunicationDbContext db,IIntegrationEventPublisher events,IHubContext<ChatHub> hub,CancellationToken ct)
    {
        if(string.IsNullOrWhiteSpace(request.Message))return Results.BadRequest(new{message="Message is required."});
        var conversation=await AuthorizedConversation(conversationId,scope,db,ct); if(conversation is null)return Results.Forbid();
        var entity=ChatMessageEntity.Create(conversation.TenantId,conversationId,scope.UserId,request.Message.Trim()); await db.ChatMessages.AddAsync(entity,ct); await db.SaveChangesAsync(ct);
    ...
    private static async Task<ChatConversationEntity?> AuthorizedConversation(Guid id,ITenantScope scope,ICommunicationDbContext db,CancellationToken ct)
    {
        var c=await db.ChatConversations.AsNoTracking().SingleOrDefaultAsync(x=>x.ChatConversationId==id&&x.IsActive,ct); if(c is null)return null;
        if(!scope.IsSuperAdmin && c.TenantId!=scope.TenantId)return null;
        if(scope.IsSuperAdmin)return c;
        return await db.ChatParticipants.AsNoTracking().AnyAsync(x=>x.TenantId==c.TenantId&&x.ConversationId==id&&x.UserId==scope.UserId&&x.IsActive,ct)?c:null;
    }
}
