...
public static class ChatEndpoints
{
    ...
    private static async Task<IResult> ConversationsAsync(ITenantScope scope,IApplicationDbContext db,CancellationToken ct)
    {
        ...
        var rows=await db.Set<ChatConversationEntity>().AsNoTracking().Where(x=>participantIds.Contains(x.Id) && x.IsActive)
            .OrderByDescending(x=>x.CreatedAt).Select(x=>new { x.TenantId,x.Id,x.Title,x.ConversationType,x.CreatedByUserId,x.IsClosed }).ToListAsync(ct);
    ...
    private static async Task<IResult> MessagesAsync(Guid conversationId,ITenantScope scope,IApplicationDbContext db,CancellationToken ct)
    {
        ...
        var rows=await db.Set<ChatMessageEntity>().AsNoTracking().Where(x=>x.ConversationId==conversationId && x.TenantId==conversation.TenantId && x.IsActive && !x.IsDeleted)
            .OrderBy(x=>x.SentAt).Select(x=>new{x.TenantId,x.Id,x.ConversationId,x.SenderUserId,x.Message,x.SentAt,x.EditedAt}).ToListAsync(ct);
        return Results.Ok(rows);
    }
    ...
    private static async Task<IResult> CreateAsync(CreateConversationRequest request,ITenantScope scope,IApplicationDbContext db,CancellationToken ct)
    {
        ...
        foreach(var p in participants) await db.Set<ChatParticipantEntity>().AddAsync(ChatParticipantEntity.Create(tenant.Value,entity.Id,p.UserId,p.Role),ct);
        await db.SaveChangesAsync(ct); return Results.Created($"/api/communication/chat/conversations/{entity.Id}",new{entity.TenantId,entity.Id,entity.Title,entity.ConversationType});
    }
    ...
    {
        var c=await db.Set<ChatConversationEntity>().AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id&&x.IsActive,ct); if(c is null)return null;
