...
public static class CreateMessageReceipt
{
    ...
    public interface ICreateMessageReceiptCommand
    {
        Task AddAsync(
    ...
    internal sealed class CreateMessageReceiptCommand(ICommunicationDbContext dbContext) : ICreateMessageReceiptCommand
    {
    ...
    public sealed class Handler(ICreateMessageReceiptCommand command, IBusinessNumberGenerator numberGenerator)
        : IRequestHandler<Request, Result<Response>>
    {
        public async Task<Result<Response>> HandleAsync(
            ...
            CancellationToken cancellationToken)
        {
            var code = await numberGenerator.NextAsync("MessageReceipt", "MR", request.TenantId, 3, cancellationToken);

            var entity = MessageReceiptEntity.Create(
                request.TenantId,
                code,
                request.Name);
