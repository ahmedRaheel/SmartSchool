...
public static class UpdateWorkflowDefinition
{
	...
	internal sealed class UpdateWorkflowDefinitionDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateWorkflowDefinition
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR workflow_definition_id <> @ExcludingId)
					);
					""";
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(WorkflowDefinitionEntity), request.Code)));
			}
