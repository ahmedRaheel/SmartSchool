...
public static class UpdateStudentTransport
{
	...
	internal sealed class UpdateStudentTransportDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateStudentTransport
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR student_transport_id <> @ExcludingId)
					);
					""";
		...
		public async Task<StudentTransportEntity?> GetByIdAsync(
				...
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<StudentTransportEntity>(
					new CommandDefinition(
						sql,
						new
						{
