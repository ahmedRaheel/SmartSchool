...
public static class CreateRoute
{
    ...
    public interface ICreateRouteCommand
    {
        Task AddAsync(
                RouteEntity entity,
    ...
    internal sealed class CreateRouteCommand(ITransportDbContext dbContext) : ICreateRouteCommand
    {
    ...
    public sealed class Handler(ICreateRouteCommand command, IBusinessNumberGenerator numberGenerator)
        : IRequestHandler<Request, Result<Response>>
    {
        public async Task<Result<Response>> HandleAsync(
            ...
            CancellationToken cancellationToken)
        {
            var code = await numberGenerator.NextAsync("Route", "ROU", request.TenantId, 3, cancellationToken);

            var entity = RouteEntity.Create(
                request.TenantId,
                code,
                request.Name);
