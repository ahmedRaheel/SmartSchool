...
public static class CreateTeachingRecommendation
{
	...
	internal sealed class CreateTeachingRecommendationDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : ICreateTeachingRecommendation
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR teaching_recommendation_id <> @ExcludingId)
					);
					""";
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(TeachingRecommendationEntity), request.Code)));
			}
