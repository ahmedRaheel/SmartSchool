...
public static class PredictPayrollAnomaly
{
    public sealed record Request(PayrollPredictionRequest Input) : IRequest<Response>;
    ...
    internal sealed class PredictPayrollAnomalyCommand(IAIPredictionDbContext dbContext) : IPredictPayrollAnomalyCommand
    {
        ...
        {
            var entity = MlPredictionResultEntity.Create(tenantId, result.Kind.ToString(), result.Score, result.Probability, result.RiskLevel, result.Outcome, result.Confidence, result.ModelVersion, result.UsedMachineLearning, JsonSerializer.Serialize(result.Factors), null, null, request.Input.EmployeeId);
            await dbContext.MlPredictionResults.AddAsync(entity, cancellationToken);
            await dbContext.SaveChangesAsync(cancellationToken);
        }
    }

    public sealed class Handler(IPredictionSuiteService service, IPredictPayrollAnomalyCommand command) : IRequestHandler<Request, Response>
    {
    ...
    {
        endpoints.MapPost("/api/aiprediction/payroll/anomaly", async (PayrollPredictionRequest request, IMediator mediator, CancellationToken cancellationToken) => Results.Ok(await mediator.SendAsync<Request, Response>(new Request(request), cancellationToken))).WithTags("AI Prediction").RequireAuthorization();
    }
}
