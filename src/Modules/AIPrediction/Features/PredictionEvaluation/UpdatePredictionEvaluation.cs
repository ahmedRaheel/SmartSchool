...
public static class UpdatePredictionEvaluation
{
	/// <summary>
	...
	public interface IUpdatePredictionEvaluation
	{
		Task UpdateAsync(
	...
	internal sealed class UpdatePredictionEvaluationDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdatePredictionEvaluation
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR prediction_evaluation_id <> @ExcludingId)
					);
					""";
	...
	public sealed class Handler(IUpdatePredictionEvaluation dataAccess)
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(PredictionEvaluationEntity), request.Code)));
			}
	...
	public static IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder endpoints)
	{
		endpoints.MapPut(
			...
			.WithName("UpdatePredictionEvaluation")
