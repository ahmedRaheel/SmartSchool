...
public static class GetPredictionEvidencePage
{
    ...
    {
        public async Task<Result<PagedResult<PredictionEvidence>>> HandleAsync(
            Query query,
            CancellationToken cancellationToken)
        {
            ...
            return Result<PagedResult<PredictionEvidence>>.Success(result);
        }
    }
    ...
    {
        endpoints.MapGet(
                "/api/aiprediction/prediction-evidence",
                async (
                    Guid tenantId,
                    int page,
                    int pageSize,
