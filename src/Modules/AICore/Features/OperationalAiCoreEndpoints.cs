...
public static class OperationalAiCoreEndpoints
{
    ...
    private static async Task<IResult> IndexAsync(IndexKnowledgeRequest r,ITenantScope scope,KnowledgeChunkWriter command,
    ...
    private static async Task<IResult> ExecuteAsync(ExecuteRequest r,ITenantScope scope,IDbConnectionFactory db,
        IOllamaClient ollama,IOptionsMonitor<OllamaRagOptions> options,AiExecutionLogWriter logs,IIntegrationEventPublisher events,TimeProvider timeProvider,CancellationToken ct)
    {
