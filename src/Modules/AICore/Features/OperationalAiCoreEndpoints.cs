...
public static class OperationalAiCoreEndpoints
{
        ...
        IHttpClientFactory clients,IConfiguration cfg,IAiExecutionLogCommand logs,IIntegrationEventPublisher events,CancellationToken ct)
    {
        ...
        var model=cfg["AI:Ollama:ChatModel"]??"qwen3:1.7b";
    ...
    {
        try{var h=clients.CreateClient("Ollama");h.BaseAddress=new Uri(Base(cfg));var r=await h.GetAsync("api/tags",ct);return Results.Ok(new{ollama=r.IsSuccessStatusCode,pgvector=true});}
    ...
    private static async Task<float[]> Embed(string text, IHttpClientFactory clients, IConfiguration cfg, CancellationToken ct)
    {
        var h = clients.CreateClient("Ollama");
        h.BaseAddress = new Uri(Base(cfg));
        var r = await h.PostAsJsonAsync("api/embed", new
        {
            model = cfg["AI:Ollama:EmbeddingModel"] ?? "nomic-embed-text",
            input = text
        }, ct);
        r.EnsureSuccessStatusCode();
        var result = await r.Content.ReadFromJsonAsync<EmbeddingResponse>(cancellationToken: ct);
    ...
    private static async Task<string> Generate(string prompt,IHttpClientFactory clients,IConfiguration cfg,CancellationToken ct){var h=clients.CreateClient("Ollama");h.BaseAddress=new Uri(Base(cfg));var r=await h.PostAsJsonAsync("api/generate",new{model=cfg["AI:Ollama:ChatModel"]??"qwen3:1.7b",prompt,stream=false},ct);r.EnsureSuccessStatusCode();return (await r.Content.ReadFromJsonAsync<GenerateResponse>(cancellationToken:ct))?.Response??"";}
