...
public static class CreateKnowledgeChunk
{
	...
	internal sealed class CreateKnowledgeChunkDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : ICreateKnowledgeChunk
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR knowledge_chunk_id <> @ExcludingId)
					);
					""";
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(KnowledgeChunkEntity), request.Code)));
			}
