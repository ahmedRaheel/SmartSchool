...
/// </summary>
public sealed class ToolDefinitionQuery(IDbConnectionFactory connectionFactory) : IToolDefinitionQuery
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<ToolDefinitionEntity>(
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT EXISTS (
                ...
                WHERE tenant_id = @TenantId
                  AND code = @Code
                  AND (@ExcludingId IS NULL OR tool_definition_id <> @ExcludingId)
            );
            """;
