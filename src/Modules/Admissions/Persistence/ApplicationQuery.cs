...
/// </summary>
public sealed class ApplicationQuery(IDbConnectionFactory connectionFactory) : IApplicationQuery
{
	...
	public async Task<PagedResult<ApplicationEntity>> GetPageAsync(
		Guid tenantId,
		int page,
		int pageSize,
		CancellationToken cancellationToken)
	{
		...
		var items = (await connection.QueryAsync<ApplicationEntity>(
			new CommandDefinition(
				pageSql,
				parameters,
				cancellationToken: cancellationToken))).ConfigureAwait(false)
		...
		return new PagedResult<ApplicationEntity>(
			items,
			page,
			pageSize,
			totalCount);
		...
		CancellationToken cancellationToken)
	{
		const string sql = """
			SELECT EXISTS (
				...
				WHERE tenant_id = @TenantId
				  AND code = @Code
				  AND (@ExcludingId IS NULL OR application_id <> @ExcludingId)
			);
			""";
