...
public static class UpdateAdmissionDecision
{
	...
	internal sealed class UpdateAdmissionDecisionDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateAdmissionDecision
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR admission_decision_id <> @ExcludingId)
					);
					""";
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(AdmissionDecisionEntity), request.Code)));
			}
