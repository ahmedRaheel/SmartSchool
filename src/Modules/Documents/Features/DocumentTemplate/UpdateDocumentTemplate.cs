...
public static class UpdateDocumentTemplate
{
	...
	internal sealed class UpdateDocumentTemplateDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateDocumentTemplate
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR document_template_id <> @ExcludingId)
					);
					""";
		...
		public async Task<DocumentTemplateEntity?> GetByIdAsync(
				...
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<DocumentTemplateEntity>(
					new CommandDefinition(
						sql,
						new
						{
