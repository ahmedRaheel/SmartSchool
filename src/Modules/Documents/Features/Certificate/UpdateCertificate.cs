...
public static class UpdateCertificate
{
	...
	internal sealed class UpdateCertificateDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateCertificate
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR certificate_id <> @ExcludingId)
					);
					""";
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(CertificateEntity), request.Code)));
			}
