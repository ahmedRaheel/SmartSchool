...
{
    public sealed record Request(DocumentOwnerType OwnerType, string Name, string? Description, Guid? TenantId = null) : IRequest<Result<Response>>;
    ...
    public static IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder endpoints)
    {
        endpoints.MapPost("/api/documents/document-types", async (Request request, IMediator mediator, CancellationToken cancellationToken) =>
            ...
            .WithName("CreateDocumentType").WithTags(ModuleConstants.Name).RequireAuthorization(SmartSchoolPolicies.SchoolAdministration);
        return endpoints;
    }
}
