...
public static class UpdateGeneratedQuiz
{
	/// <summary>
	...
	public interface IUpdateGeneratedQuiz
	{
		Task UpdateAsync(
	...
	internal sealed class UpdateGeneratedQuizDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateGeneratedQuiz
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR generated_quiz_id <> @ExcludingId)
					);
					""";
		...
		public async Task<GeneratedQuizEntity?> GetByIdAsync(
				Guid tenantId,
				Guid id,
				CancellationToken cancellationToken)
			{
	...
	public sealed class Handler(IUpdateGeneratedQuiz dataAccess)
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(GeneratedQuizEntity), request.Code)));
			}
