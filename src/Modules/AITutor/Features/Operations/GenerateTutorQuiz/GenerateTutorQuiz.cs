...
{
 public sealed record Request(Guid? TenantId,Guid StudentId,string Subject,string Topic,int QuestionCount=5,string Difficulty="adaptive",Guid? SubjectId=null):IRequest<Response?>;  public sealed class Validator : AbstractValidator<Request>
 {
  public Validator(){RuleFor(x=>x.StudentId).NotEmpty();RuleFor(x=>x.Subject).NotEmpty().MaximumLength(150);RuleFor(x=>x.Topic).NotEmpty().MaximumLength(250);RuleFor(x=>x.QuestionCount).InclusiveBetween(1,20);RuleFor(x=>x.Difficulty).NotEmpty().MaximumLength(30);}
 }
 public sealed record Response(Guid QuizId,object Questions);
 ...
 internal sealed class GenerateTutorQuizCommand(IAITutorDbContext dbContext, IBusinessNumberGenerator numberGenerator):IGenerateTutorQuizCommand{public async Task<GeneratedQuizEntity>AddAsync(Guid tenantId,Request request,string raw,CancellationToken cancellationToken){var entity=GeneratedQuizEntity.Create(tenantId,await numberGenerator.NextAsync("GeneratedQuiz", "QUIZ", tenantId, 3, cancellationToken),$"{request.Subject} - {request.Topic}",JsonSerializer.Serialize(new{request.StudentId,request.SubjectId,request.Subject,request.Topic,request.Difficulty,questionsJson=raw}),studentId:request.StudentId,subjectId:request.SubjectId,topic:request.Topic,difficulty:request.Difficulty);await dbContext.GeneratedQuizs.AddAsync(entity,cancellationToken);await dbContext.SaveChangesAsync(cancellationToken);return entity;}}
