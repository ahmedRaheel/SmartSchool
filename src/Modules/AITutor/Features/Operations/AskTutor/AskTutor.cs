...
public static class AskTutor
{
    ...
    internal sealed class AskTutorCommand(IAITutorDbContext dbContext, IBusinessNumberGenerator numberGenerator):IAskTutorCommand { public async Task<TutorMessageEntity>AddAsync(Guid tenantId,string role,string payload,CancellationToken cancellationToken){var entity=TutorMessageEntity.Create(tenantId,await numberGenerator.NextAsync("TutorMessage", "TMSG", tenantId, 3, cancellationToken),role,payload);await dbContext.TutorMessages.AddAsync(entity,cancellationToken);await dbContext.SaveChangesAsync(cancellationToken);return entity;}}
