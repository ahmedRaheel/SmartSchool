...
{
 public sealed record Request(Guid? TenantId,Guid StudentId,string Subject,string Topic,double MasteryScore,Guid? SubjectId=null):IRequest<Response?>;  public sealed class Validator : AbstractValidator<Request>
 {
  public Validator(){RuleFor(x=>x.StudentId).NotEmpty();RuleFor(x=>x.Subject).NotEmpty().MaximumLength(150);RuleFor(x=>x.Topic).NotEmpty().MaximumLength(250);RuleFor(x=>x.MasteryScore).InclusiveBetween(0,1);}
 }
 public sealed record Response(Guid RecommendationId,string Recommendation);
 ...
 internal sealed class GenerateLearningRecommendationCommand(IAITutorDbContext dbContext, IBusinessNumberGenerator numberGenerator):IGenerateLearningRecommendationCommand{public async Task<LearningRecommendationEntity>AddAsync(Guid tenantId,Request request,string recommendation,CancellationToken cancellationToken){var entity=LearningRecommendationEntity.Create(tenantId,await numberGenerator.NextAsync("LearningRecommendation", "REC", tenantId, 3, cancellationToken),$"{request.Subject} recommendation",JsonSerializer.Serialize(new{request.StudentId,request.SubjectId,request.Subject,request.Topic,request.MasteryScore,recommendation}),studentId:request.StudentId,subjectId:request.SubjectId,topic:request.Topic,recommendationType:"AI_TUTOR",recommendationText:recommendation,priority:1,status:"ACTIVE");await dbContext.LearningRecommendations.AddAsync(entity,cancellationToken);await dbContext.SaveChangesAsync(cancellationToken);return entity;}}
