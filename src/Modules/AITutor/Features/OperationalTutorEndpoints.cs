...
public static class OperationalTutorEndpoints
{
    ...
    private static async Task<IResult> Start(StartSessionRequest r,ITenantScope scope,TutorSessionWriter sessions,TutorConversationWriter conversations,CancellationToken ct)
    {
    ...
    private static async Task<IResult> Ask(AskRequest r,ITenantScope scope,TutorMessageWriter messages,IOllamaClient ollama,IIntegrationEventPublisher events,CancellationToken ct)
    {
        var t=Tenant(scope,r.TenantId);if(!t.HasValue)return Results.BadRequest(new{message="Tenant required."});
...
""";
    ...
    private static async Task<IResult> Quiz(QuizRequest r,ITenantScope scope,GeneratedQuizWriter quizzes,IOllamaClient ollama,IIntegrationEventPublisher events,CancellationToken ct)
    {
        var t=Tenant(scope,r.TenantId);if(!t.HasValue)return Results.BadRequest(new{message="Tenant required."});
...
""";
    ...
    private static async Task<IResult> Recommend(RecommendationRequest r,ITenantScope scope,LearningRecommendationWriter recommendations,IOllamaClient ollama,CancellationToken ct)
    {
        var t=Tenant(scope,r.TenantId);if(!t.HasValue)return Results.BadRequest(new{message="Tenant required."});
