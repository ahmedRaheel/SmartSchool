...
public sealed class LearningRecommendationQuery(
	...
	IDbConnectionFactory connectionFactory) : ILearningRecommendationQuery
{
		...
		CancellationToken cancellationToken)
	{
		return dbContext
			...
			.SingleOrDefaultAsync(
				entity => entity.TenantId == tenantId && entity.LearningRecommendationId == id,
				cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		var items = (await connection.QueryAsync<LearningRecommendationEntity>(
			new CommandDefinition(
				pageSql,
				parameters,
				cancellationToken: cancellationToken)))
		...
		return dbContext
			...
			.AnyAsync(
				entity =>
					...
					&& (!excludingId.HasValue || (excludingId.HasValue && entity.LearningRecommendationId != excludingId.Value)),
				cancellationToken);
	}
}
