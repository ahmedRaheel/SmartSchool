...
public static class UpdateItem
{
	...
	internal sealed class UpdateItemDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdateItem
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR item_id <> @ExcludingId)
					);
					""";
		...
		public async Task<ItemEntity?> GetByIdAsync(
				Guid tenantId,
				Guid id,
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<ItemEntity>(
					new CommandDefinition(
						sql,
						new
						{
	...
	public sealed class Handler(IUpdateItem dataAccess)
