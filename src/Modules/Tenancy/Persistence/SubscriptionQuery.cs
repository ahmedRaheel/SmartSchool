...
public sealed class SubscriptionQuery(
	...
	IDbConnectionFactory connectionFactory) : ISubscriptionQuery
{
		...
		CancellationToken cancellationToken)
	{
		return dbContext
			...
			.SingleOrDefaultAsync(
				entity => entity.TenantId == tenantId && entity.SubscriptionId == id,
				cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		var items = (await connection.QueryAsync<SubscriptionEntity>(
			new CommandDefinition(
				pageSql,
				parameters,
				cancellationToken: cancellationToken)))
		...
		return dbContext
			...
			.AnyAsync(
				entity =>
					...
					&& (!excludingId.HasValue || (excludingId.HasValue && entity.SubscriptionId != excludingId.Value)),
				cancellationToken);
	}
}
