...
public static class CreateRoleAssignment
{
	...
	public interface ICreateRoleAssignment
	{
		Task AddAsync(
	...
	internal sealed class CreateRoleAssignmentDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : ICreateRoleAssignment
	{
	...
	public sealed class Handler(ICreateRoleAssignment dataAccess)
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(RoleAssignmentEntity), request.Code)));
			}

			var entity = RoleAssignmentEntity.Create(
				request.TenantId,
				request.Code,
				request.Name);
