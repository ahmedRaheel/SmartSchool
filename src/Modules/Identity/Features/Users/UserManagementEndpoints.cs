...
public static class UserManagementEndpoints
{
		...
		[FromServices] UserManager<SmartSchoolUser> userManager, CancellationToken cancellationToken)
	{
		...
		if (!currentUser.IsSuperAdmin && effectiveTenant is null)
			return Results.Forbid();

		var query = userManager.Users.AsNoTracking();
		if (effectiveTenant.HasValue)
			query = query.Where(x => x.TenantId == effectiveTenant.Value);
		...
		foreach (var user in users)
			result.Add(await ToResponseAsync(user, userManager));
		...
		[FromServices] ILoggerFactory loggerFactory)
	{
		...
		if (!superAdmin && callerTenant != request.TenantId)
			return Results.Forbid();
	...
	private static async Task<IResult> SetRolesAsync(Guid id, SetRolesRequest request, [FromServices] ICurrentUser currentUser, [FromServices] UserManager<SmartSchoolUser> userManager)
	{
		...
		if (!currentUser.IsSuperAdmin && user.TenantId != currentUser.TenantId)
			return Results.Forbid();
		if (!currentUser.IsSuperAdmin && request.Roles.Any(r => !SchoolRoles.Contains(r) || r.Equals(SmartSchoolRoles.SuperAdmin, StringComparison.OrdinalIgnoreCase)))
			return Results.Forbid();
		var current = await userManager.GetRolesAsync(user);
		var remove = await userManager.RemoveFromRolesAsync(user, current);
		if (!remove.Succeeded)
			return Results.ValidationProblem(ToErrors(remove));
