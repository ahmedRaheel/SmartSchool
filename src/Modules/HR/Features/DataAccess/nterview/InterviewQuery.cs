...
/// </summary>
public sealed class InterviewQuery(IDbConnectionFactory connectionFactory) : IInterviewQuery
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<InterviewEntity>(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var items = (await connection.QueryAsync<InterviewEntity>(
            new CommandDefinition(
                pageSql,
                parameters,
                cancellationToken: cancellationToken)).ConfigureAwait(false))
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT EXISTS (
                ...
                WHERE tenant_id = @TenantId
                  AND code = @Code
                  AND (@ExcludingId IS NULL OR interview_id <> @ExcludingId)
            );
            """;
