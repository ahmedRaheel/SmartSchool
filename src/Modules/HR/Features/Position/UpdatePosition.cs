...
public static class UpdatePosition
{
	...
	internal sealed class UpdatePositionDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : IUpdatePosition
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR position_id <> @ExcludingId)
					);
					""";
		...
		public async Task<PositionEntity?> GetByIdAsync(
				...
				CancellationToken cancellationToken)
			{
				...
				return await connection.QuerySingleOrDefaultAsync<PositionEntity>(
					new CommandDefinition(
						sql,
						new
						{
		...
		: IRequestHandler<Request, Result<Response>>
	{
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(PositionEntity), request.Code)));
			}
