...
public static class UpdatePosition
{
    public sealed record Request(
        ...
        string Code,
        string Name,
        bool IsActive) : IRequest<Result<PositionResponse>>;
    ...
    public sealed class Handler(
        IPositionQuery entityQuery,
        IPositionCommand entityCommand,
        IValidator<Request> validator)
        : IRequestHandler<Request, Result<PositionResponse>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!validation.IsValid)
            {
                ...
                return Result<PositionResponse>.Failure(Error.Validation(message));
            }
            ...
            if (exists)
            {
                return Result<PositionResponse>.Failure(
                    Error.Conflict(
                        ErrorMessages.DuplicateCode(nameof(Position), request.Code)));
            }
    ...
    public static IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder endpoints)
    {
        endpoints.MapPut(
                ...
                async (Guid id, Request request, IMediator mediator, CancellationToken cancellationToken) =>
                {
                    ...
                    var result = await mediator.SendAsync<Request, Result<PositionResponse>>(
