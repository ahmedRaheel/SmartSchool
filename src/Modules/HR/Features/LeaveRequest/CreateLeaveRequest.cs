...
public static class CreateLeaveRequest
{
	...
	internal sealed class CreateLeaveRequestDataAccess(
		IApplicationDbContext dbContext,
		IDbConnectionFactory connectionFactory) : ICreateLeaveRequest
	{
				...
				const string sql = """
					SELECT EXISTS (
						...
						WHERE tenant_id = @TenantId
						  AND code = @Code
						  AND (@ExcludingId IS NULL OR leave_request_id <> @ExcludingId)
					);
					""";
			...
			CancellationToken cancellationToken)
		{
			...
			if (exists)
			{
				return Result<Response>.Failure(
					Error.Conflict(
						ErrorMessages.DuplicateCode(nameof(LeaveRequestEntity), request.Code)));
			}
