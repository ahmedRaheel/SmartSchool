...
internal static class SmokeTestIdentityFixtureEndpoints
{
    ...
    private static IResult Probe(HttpContext context, IConfiguration configuration) =>
        Authorized(context, configuration)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!IsValidRunOwnedEmail(request.RunId, request.Email))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!IsValidRunOwnedEmail(runId, user.Email))
        {
    ...
    private static bool IsValidRunOwnedEmail(string runId, string? email)
    {
        ...
        return email.StartsWith(runId + ".", StringComparison.OrdinalIgnoreCase)
            && email.EndsWith(TestEmailSuffix, StringComparison.OrdinalIgnoreCase);
    }
